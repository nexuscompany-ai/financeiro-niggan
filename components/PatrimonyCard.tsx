    ...
    <div className="px-4 mb-3">
      <div className="bg-white border border-neutral-100 rounded-xl overflow-hidden">
        ...
        <div onClick={() => setExpanded(!expanded)}
          className="p-4 flex items-center justify-between cursor-pointer active:bg-neutral-50">
          <div>
            <p className="text-xs font-bold text-neutral-500 uppercase">Patrimônio</p>
            <p className="text-xl font-bold text-olive-900">{formatCurrency(total)}</p>
            <div className="flex items-center gap-2 mt-1">
              <div className="flex-1 w-24 bg-neutral-100 rounded-full h-1.5">
          ...
          <span className="text-neutral-400 text-lg">{expanded ? '▲' : '▼'}</span>
        </div>
              ...
              <div key={p.account} className="border-b border-neutral-50 last:border-0">
                {editing === p.account ? (
                  <div className="px-4 py-3 flex gap-2 items-center">
                    ...
                    <div className="relative">
                      ...
                      <input type="number" value={value} onChange={e => setValue(e.target.value)}
                        className="pl-7 pr-2 py-1.5 bg-neutral-100 rounded-lg text-sm w-28 font-bold"
                  ...
                  <div onClick={() => { setEditing(p.account); setValue(p.balance.toString()) }}
                    ...
                    <p className="text-sm font-bold text-neutral-900">{formatCurrency(p.balance)}</p>

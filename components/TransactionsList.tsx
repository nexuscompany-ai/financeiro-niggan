...
export default function TransactionsList({ hidden = false }: { hidden?: boolean }) {
  ...
  const filtered = transactions.filter(t => {
    ...
    if (filter === 'month') { const d = new Date(t.date+'T12:00:00'); return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear() }
  ...
  const startEdit = (tx: Transaction) => { setEditingId(tx.id); setEditData({type:tx.type,category:tx.category,amount:tx.amount,description:tx.description,date:tx.date}); setExpandedId(null) }
  const saveEdit = () => { if (!editingId||!editData.amount||editData.amount<=0) return; updateTransaction(editingId,editData); setEditingId(null); setEditData({}) }
  ...
  return (
    <div>
      <div className="px-4 mb-3 flex gap-2 overflow-x-auto pb-1" style={{scrollbarWidth:'none'}}>
        {([['today','Hoje'],['7days','7 dias'],['month','Mês'],['all','Tudo'],['income','↓ Entradas'],['expense','↑ Saídas'],['investment','📈 Invest.']] as [FilterType,string][]).map(([k,l]) => (
          <button key={k} onClick={()=>setFilter(k)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${filter===k?'bg-olive-700 text-white':'bg-neutral-100 text-neutral-600'}`}>{l}</button>
        ))}
      </div>

      <div className="px-4 pb-32">
        {filtered.length===0 ? (
          <div className="py-12 text-center"><p className="text-4xl mb-2">📭</p><p className="text-sm text-neutral-500">Nenhuma transação</p></div>
        ) : sortedDates.map(date => (
          <div key={date} className="mb-4">
            <div className="flex items-center gap-2 mb-2">
              <p className="text-xs font-bold text-neutral-400">{formatDate(date)}</p>
              <div className="flex-1 h-px bg-neutral-100" />
              <p className="text-xs font-bold text-neutral-400">{fmt(grouped[date].reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0))}</p>
            </div>
                  ...
                  {editingId===tx.id ? (
                    <div className="p-3 space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input type="number" step="0.01" min="0" value={editData.amount||''} onChange={e=>setEditData(d=>({...d,amount:parseFloat(e.target.value)}))}
                      ...
                      <div className="flex gap-2 pt-1">
                        <button onClick={()=>{setEditingId(null);setEditData({})}} className="flex-1 py-2.5 bg-neutral-100 text-neutral-600 rounded-lg text-sm font-medium">Cancelar</button>
                        <button onClick={saveEdit} className="flex-1 py-2.5 bg-olive-700 text-white rounded-lg text-sm font-bold">✅ Salvar</button>
                      </div>
                    </div>
                  ) : (
                    <>
                      <div onClick={()=>setExpandedId(expandedId===tx.id?null:tx.id)} className="flex items-center gap-3 p-3 cursor-pointer active:bg-neutral-50">
                        <span className="text-xl flex-shrink-0">{CATEGORY_EMOJI[tx.category]||(tx.type==='income'?'💰':tx.type==='investment'?'📈':'💸')}</span>
                        ...
                        <p className={`text-sm font-bold flex-shrink-0 ${typeColor(tx.type)}`}>{tx.type==='income'?'+':'-'}{fmt(tx.amount)}</p>
                      </div>
                      {expandedId===tx.id && (
                        <div className="border-t border-neutral-100 px-3 py-2 flex gap-2 bg-neutral-50">
                          <button onClick={()=>startEdit(tx)} className="flex-1 py-2 bg-olive-100 text-olive-800 rounded-lg text-xs font-bold active:bg-olive-200">✏️ Editar</button>
                          {confirmDelete===tx.id ? (
                            <>
                              <button onClick={()=>{removeTransaction(tx.id);setExpandedId(null);setConfirmDelete(null)}} className="flex-1 py-2 bg-red-600 text-white rounded-lg text-xs font-bold">Confirmar</button>
                              <button onClick={()=>setConfirmDelete(null)} className="flex-1 py-2 bg-neutral-200 text-neutral-600 rounded-lg text-xs font-medium">Cancelar</button>
                            </>
                          ) : (
                            <button onClick={()=>setConfirmDelete(tx.id)} className="flex-1 py-2 bg-red-50 text-red-600 rounded-lg text-xs font-bold active:bg-red-100">🗑️ Deletar</button>
                          )}
                        </div>
                      )}
                    </>

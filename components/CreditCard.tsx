    ...
    <div className="px-4 mb-3">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={()=>setShowForm(!showForm)}
          ...
          {showForm ? <><Icon name="close" size={11} color="#544C31" /> Fechar</> : <><Icon name="plus" size={11} color="#F0D98A" /> Nova parcela</>}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-2xl p-4 mb-3 shadow-card animate-slide-up" style={{ border:'1px solid #F0EFE9' }}>
          ...
          <form onSubmit={handleAdd} className="space-y-2.5">
            ...
            <div className="flex gap-2">
              ...
              <select value={form.installments} onChange={e=>setForm(f=>({...f,installments:e.target.value}))}
                className="w-28 px-3 py-3 rounded-xl text-sm cursor-pointer"
                style={{ background:'#F8F8F6', border:'1.5px solid #E5E3D8' }}>
            ...
            <button type="submit" className="w-full py-3 rounded-xl font-semibold text-sm pressable"
              style={{ background:'#3D3822', color:'#F0D98A' }}>Confirmar</button>
          </form>
        </div>
      )}
      ...
      <div className="space-y-3">
        {[
          { card:'C6' as const, total:totalC6, cls:'card-c6', accent:'#C9A84C', name:'C6 Bank · Black' },
          { card:'Nubank' as const, total:totalNu, cls:'card-nubank', accent:'rgba(255,255,255,0.7)', name:'Nubank · Roxo' },
        ].map(({ card, total, cls, accent, name }) => {
          const purchases  = cardPurchases(card)
          const isExpanded = expandedCard===card

          return (
            <div key={card} className={`${cls} rounded-2xl overflow-hidden`} style={{ boxShadow:'0 8px 32px rgba(0,0,0,0.24)' }}>
              <div onClick={()=>setExpandedCard(isExpanded?null:card)} className="p-5 cursor-pointer pressable">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="w-9 h-6 rounded mb-3 opacity-90"
                      style={{ background:card==='C6'?'linear-gradient(135deg,#C9A84C,#F0D98A)':'rgba(255,255,255,0.25)' }} />
                    <p className="font-display font-semibold text-sm" style={{ color:accent }}>{name}</p>
                    <p className="text-xs mt-0.5" style={{ color:'rgba(255,255,255,0.35)' }}>Vence {dueDate(card)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs mb-1" style={{ color:'rgba(255,255,255,0.35)' }}>Fatura</p>
                    <p className="font-display font-bold text-xl tabular" style={{ color:total>0?'#F87171':'#4ADE80' }}>
                      {fmt(total)}
                    </p>
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  <p className="text-xs tracking-widest" style={{ color:'rgba(255,255,255,0.15)' }}>•••• •••• •••• ••••</p>
                  <div className="flex items-center gap-2">
                    <p className="text-xs" style={{ color:'rgba(255,255,255,0.3)' }}>{purchases.length} compra{purchases.length!==1?'s':''}</p>
                    <Icon name={isExpanded?'chevronUp':'chevronDown'} size={14} color="rgba(255,255,255,0.3)" />
                  </div>
                </div>
              </div>

              {isExpanded && (
                <div style={{ background:'rgba(0,0,0,0.2)', borderTop:'1px solid rgba(255,255,255,0.07)' }}>
                  {purchases.length===0 ? (
                    <p className="text-center py-4 text-xs" style={{ color:'rgba(255,255,255,0.25)' }}>Nenhuma compra registrada</p>
                  ) : purchases.map(p => (
                    <div key={p.id} className="px-5 py-3 flex items-center justify-between"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {confirmRemove===p.id ? (
                          ...
                          <button onClick={()=>setConfirmRemove(p.id)} className="pressable" style={{ color:'rgba(255,255,255,0.2)' }}>
                            <Icon name="close" size={16} color="rgba(255,255,255,0.2)" />
                          </button>
                        )}
                      </div>
                    </div>
